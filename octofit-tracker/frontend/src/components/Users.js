          ...
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="userDetailModalLabel">
                {selectedItem ? selectedItem.username || selectedItem.name || 'User Details' : 'User Details'}
              </h5>
              <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close" />
            </div>
            <div className="modal-body">
              {selectedItem ? <pre>{JSON.stringify(selectedItem, null, 2)}</pre> : <p>No user selected.</p>}
            </div>
